...
| REQ-EVENT-001 | Bounded outbox relay and timeline consumer | event_envelope/event_delivery_receipt | bounded local outbox delivery state plus authoritative timeline | M1-07 | claim crash, post-effect crash, historical token reuse, exact lease deadline, mismatched receipt, max attempts, poison, DLQ, telemetry sink failure, ordering and cross-tenant tests | retry only after lease/backoff; dead letter blocks only its aggregate; no duplicate timeline effect |
| REQ-WORKFLOW-001 | Checkpointed fake post-call workflow | post_call_workflow_command/post_call_workflow_status/workflow_enqueue_receipt/workflow_step_receipt/post_call_workflow_result | bounded local workflow store plus normative RLS persistence shape | M1-08 | composite relay crash boundaries, resume with replacement worker, lease and fencing, retry and exhaustion, cancellation, idempotency, RLS and append-only tests | no external effect; restart resumes from persisted checkpoint; loss of local store is not durable recovery |
| REQ-OPS-001 | Tenant-safe read-only operations console | interaction_session_state/event_envelope/action_intent/policy_decision/tool_execution_receipt/cost_event | lifecycle gate plus authoritative timeline replay, bounded action projection and exact cost aggregation | M1-09 | component states, XSS and TOCTOU rejection, receipt versus hypothesis semantics, accessibility smoke, lifecycle-first tenant denial and sanitized route tests | foreign and missing sessions share one 404; integrity mismatch produces no partial document; no browser auth, write path or polling in M1 |
