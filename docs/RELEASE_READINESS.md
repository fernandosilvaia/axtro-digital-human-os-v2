...
| Custos | 91 | Todo caminho pago com teto + ledger; catálogo de teste (1 Meter + 3 Products + 6 Prices) provisionado de verdade na Stripe e validado contra a API real; **novo**: alertas proativos por e-mail em 80%/100% dos 4 tetos diários (D-V2-107, migration 0031 aplicada e verificada em produção) |
| IA | 88 | Eval golden pelo adapter real; injeção de percepção fechada; **RAG do brain fechado** (D-V2-108) — `portal_search_knowledge_service` (migration 0032, aplicada e verificada em produção) espelha exatamente a busca já usada no chat de teste, agora também no caminho de vídeo (Tavus) |
