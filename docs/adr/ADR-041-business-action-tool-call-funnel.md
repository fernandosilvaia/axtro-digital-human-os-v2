...
RPC nova, `portal_business_action_call_context_service(p_tenant_id, p_agent_id, p_idempotency_key)`, migration 0054. Nota de numeração: `database/supabase-only/` termina hoje (main) em `0052_business_action_calendar_scheduling.sql`, tornando `0053` o próximo número livre à primeira vista, mas `0053` já está reservado localmente pela onda 1b-iii (`portal_google_calendar_decrypted_refresh_token_service`, RPC de leitura do refresh token decifrado do Vault), ainda não pushada nem mergeada em `main` no momento em que este ADR foi escrito. A próxima migration realmente livre, considerando esse trabalho em andamento, é `0054`. Se a ordem de merge inverter (esta RPC mergear antes da 1b-iii), renumerar seguindo o mesmo padrão já registrado em D-V2-145/146/149 no `DECISIONS_LOG.md`.
...
Rollback é imediato e sem perda: voltar `PORTAL_BUSINESS_ACTION_BRIDGE_ENABLED` para `false` bloqueia toda admissão nova sem afetar nenhum grant ou receipt já gravado (mesmo comportamento que ADR-039 já garante); reverter o código deste ADR (roteador, Server Action, RPC de leitura 0054) para uma versão anterior é seguro porque nenhuma tabela ou RPC pré-existente foi alterada, só leitura nova adicionada.
