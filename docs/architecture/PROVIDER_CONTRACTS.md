...
The ToolPort is declared and remains fail-closed until a future runtime-owned provider binding. There is no public factory for an authorization artifact, and M0-11 and M0-12 never invoke a tool adapter. M0-14 owns the complete `ActionIntent` to `PolicyDecision` to idempotent execution to `ToolExecutionReceipt` funnel through its private local fixture. A syntactically valid intent or allow decision is never authority.
