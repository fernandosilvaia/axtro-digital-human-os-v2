...
O verificador determinístico consulta somente um registro server-side injetado no startup. Ele não é OIDC nem JWT de produção. O baseline M0 aceita `session:read`, `session:write`, `provider:use` e `tool:use`; M1-07 adiciona `event:relay` e `event:observe`, e M1-08 adiciona `workflow:dispatch`, `workflow:execute` e `workflow:observe`. Esses cinco escopos operacionais são exclusivos de service identity com actor type `workflow`. Toda operação de lifecycle, timeline, Session Actor, outbox, custo, relay ou workflow que lê ou altera dados de sessão exige também a finalidade `essential_processing`; possuir o scope com apenas `provider_auth` ou `tool_auth` não concede essa autoridade. O relay recebe dispatch, não autoridade para claim ou cancelamento. Claim e checkpoint exigem execute mais `session:read`; cancellation exige somente execute e continua vinculada a `essential_processing`. Resultados post-call são `restricted`; sua leitura exige observe mais `session:read`. As finalidades registráveis permanecem `essential_processing`, `provider_auth` e `tool_auth`. Wildcard, admin, bypass e grants amplos continuam proibidos. O verificador só pode ser construído com `dev_auth_enabled=true` em `development` ou `test`. Staging, canary e produção exigem um verificador de identidade posterior e falham fechados para o fake.
