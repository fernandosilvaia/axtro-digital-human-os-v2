...
| `0048_tavus_stage_settlement_timestamp.sql` | **pendente de aplicação humana** — reparo forward-only da corrida entre criação e mutação Tavus: settle, resolve e revoke capturam o relógio de parede depois de seus locks e mantêm `updated_at` monotônico no limite original `expires_at <= updated_at + 45 minutes`. Não alonga a capability, não enfraquece a constraint e preserva RLS/grants; bootstrap e readiness exigem a capability v48. |
