...
| `0039_revoke_default_grants_rls_no_policy_tables.sql` | **não — escrita, aguardando autorização explícita do Fernando** (onda 7, D-V2-116) — fecha achado P2: 6 tabelas com RLS habilitada e zero policies (agent_brain_config, agent_video_config, conversation_transcripts, meeting_bot_sessions, tenant_cost_alerts, tenant_subscriptions) nunca revogaram o GRANT ALL padrão do Supabase pra anon/authenticated (confirmado ao vivo via `information_schema.role_table_grants` + `pg_default_acl`) — só `tenant_invites`/`user_tenant_memberships` fizeram isso desde o início. Seguro hoje (RLS+zero policy já nega tudo), é defesa-em-profundidade contra uma policy futura mal escrita valer imediatamente pra INSERT/UPDATE/DELETE também. Mesmo padrão de `revoke all on table ... from authenticated, anon, public` já usado em 0002/0006 |
