...
| `0033_fix_reported_cost_reconciliation.sql` | **NÃO aplicado ainda** — corrige `portal_log_ai_usage` (branch `provider_reported`, 0027, já aplicada): `amount_usd`/`unit_cost_usd` eram arredondados independentemente, violando `cost_events_amount_reconciliation_check` em ~90% das chamadas reais (achado P0 da auditoria 2026-08-11, verificado 3x e revalidado ao vivo contra o Postgres real). Aguardando autorização explícita do Fernando |
