...
| `0049_portal_text_preview_admission.sql` | artefato imutável restaurado do commit local `86c321a`, SHA-256 `79b24e7fdc768a30b02d3596b71799fae484043e37561ddfcd435f46076b3100`. O registro operacional relata uma aplicação histórica, mas o estado remoto atual não foi consultado nesta recuperação e permanece não confirmado. |
| `0050_meeting_terminal_notification_claim.sql` | artefato imutável restaurado do commit local `dc4df77`, SHA-256 `262e033328175f704f8cfef1cafdcb0a2ef9b9aac7e4cc86f2b33890044c7224`. Adiciona a claim atômica de notificação terminal do Recall. O registro operacional relata uma aplicação histórica, mas o estado remoto atual não foi consultado nesta recuperação e permanece não confirmado. |
...
| `0055_business_action_email_length_bound.sql` | **não aplicada ainda** (implementação local, achado da revisão adversarial ADR-041, gate humano pendente). Fecha a única lacuna de TAMANHO no funil de tool-call de negócio: `contact_email`/`google_account_email` tinham só checagem de formato (regex), sem `char_length`, diferente de todos os outros campos de texto do mesmo domínio. Adiciona `char_length(...) <= 320` (RFC 5321 sec. 4.5.3.1.3, mesmo bound já usado pela camada de aplicação em `apps/portal/src/lib/google-calendar/id-token.ts`) a quatro CHECK constraints (`portal_business_action_leads_email_chk` da 0051, `portal_business_action_proposals_email_chk`, `portal_business_action_calendar_connections_email_chk` e `portal_business_action_calendar_reservations_email_chk` da 0052, padrão widen aditivo, nunca edita as migrations originais) e republica quatro RPCs `service_role`-only com a mesma checagem adicionada ao corpo: `portal_register_business_lead_service`, `portal_propose_business_meeting_slots_service`, `portal_reserve_business_meeting_slot_service` e `portal_connect_google_calendar_service` (esta última não estava na lista original do achado, mas tinha a mesma lacuna). Não altera comportamento além do bound de tamanho, nenhuma tabela nova, nenhuma coluna nova. `portal_schema_capabilities_service()` foi deliberadamente mantida em v54, sem chave nova. Provado por `scripts/supabase-portal-integration.mjs` (extensão de `assertBusinessActionAdmissionAndLeads`/`assertBusinessActionCalendarScheduling`) contra Postgres local, cobrindo um e-mail de 320 caracteres (não regride) e um de 321 (rejeitado pela RPC e, no caso de `portal_business_action_leads`, também pela CHECK constraint via INSERT direto); nunca aplicada ao projeto hospedado. |
| `0056_schema_capability_lineage_repair.sql` | **não aplicada**. Reparo forward-only que republica `portal_schema_capabilities_service()` em v56 com a união das capabilities v48, v49, v50 e business-action v51 a v55, incluindo `businessActionEmailLengthBound`. As capabilities de ação de negócio também provam RLS habilitada e forçada, ausência de privilégios de tabela para papéis externos e execução de RPC restrita a `service_role`. Preserva as migrations anteriores. Apply remoto e verificação do estado hospedado continuam fora deste trabalho local. |
| `0057_meeting_terminal_notification_outbox.sql` | **não aplicada**. Substitui a claim-before-send por outbox durável tenant-scoped, enqueue atômico, payload congelado, revalidação de autoridade na lease e no fence, lease cercada, receipts discriminados e sem PII, retry limitado, dead letter, resolução finita de bot desconhecido e heartbeat próprio. A claim antiga passa a retornar sempre `false`. Não faz backfill histórico. Apply remoto, segredo, flag e canary continuam gates externos; agenda automática e tráfego de clientes dependem também de M6-05. |
| `0058_portal_text_preview_authority_repair.sql` | **não aplicada**. Reparo forward-only de M6-02: revoga a admissão histórica de todas as roles externas, recupera uma boundary owner-only que deriva `auth.uid()` sem aceitar `tenant_id` ou `user_id`, rejeita persistência antes de qualquer escrita, cerca todos os efeitos do preview nas gerações 0 a 9 e vincula transcript por `(tenant_id, transcript_id)`. A exclusão do transcript limpa somente a referência nos receipts append-only. Apply remoto, grant externo, ativação, relay, cleanup agendado, reconciliação operacional, provider real e rollout permanecem em M6-06. Retenção, legal hold, purge e a governanção de exclusão completa permanecem em M6-04. |
| `0059_data_governance_disposition_workflow.sql` a `0068_agent_video_config_self_service_catches_up.sql` | **não anotadas nesta tabela** (lacuna encontrada em 2026-09-18, não preenchida nesta rodada por estar fora do escopo da tarefa que a achou). O estado real de cada uma está narrado em `PROGRESS.md` e `docs/operations/DECISIONS_LOG.md`; a 0059 (M6-04, workflow de governança de dados) está confirmada **não aplicada** em produção (`PROGRESS.md`, bloco "Estado atual"), as demais precisam de conferência individual antes de confiar nesta tabela pra elas. |
| `0069_business_action_checkout_stripe_connect.sql` | **não aplicada ainda** (implementação local, ADR-040, gate humano pendente, mesma disciplina de toda migration deste porte). Quarta ação de negócio sob o `BusinessActionIntent` do ADR-039: `request_checkout`, com aprovação humana obrigatória do `tenant_admin` antes de qualquer chamada à Stripe (decisão de Fernando Silva, 2026-08-28, revisão do ADR-040: estado inicial novo `pending_approval`, anterior a `reserved`, dois estados terminais pré-Stripe novos `rejected`/`approval_expired` com prazo fixo de 72h). Adiciona `portal_business_action_checkout_connections` (conta Stripe conectada por tenant, cobrança direta Connect Standard, sem custódia de token: só `stripe_account_id`, não secreto), `portal_business_action_checkout_products` (catálogo fechado por tenant, o modelo nunca informa preço/produto por texto livre), `portal_business_action_checkout_reservations` (reserva durável no padrão ADR-036, com o estado `pending_approval` novo e snapshot completo de preço/conta/taxa de plataforma no momento da reserva, nunca recalculado depois) e `portal_business_action_checkout_stripe_event_receipts` (dedup do webhook de conta conectada, endpoint/segredo próprios, fora do escopo desta migration). `checkout_enabled` novo em `portal_business_action_agent_settings` (terceiro degrau de rollout dark por agente); `portal_set_business_action_agent_settings_service` republicada com esse parâmetro a mais (drop explícito da assinatura antiga primeiro, mesma lição do `portal_set_agent_video_config` em 0068: um parâmetro novo com default cria um segundo overload em vez de substituir). Amplia os três allowlists de `action_kind` (grants/receipts/kill switches) e o outcome de receipt (`pending_approval`, quinto valor aditivo: esta é a primeira ação de negócio cujo receipt nunca alcança `succeeded`, o desfecho real vive só na reserva). `platform_fee_bps` estruturalmente suportado por tenant, sempre nulo por padrão: o percentual em si é decisão comercial pendente do Fernando, não decidida aqui. `stripe_account_id` é `unique` (não só `tenant_id`): sem isso a resolução de tenant a partir do `account.updated` da Stripe (que não carrega `tenant_id` nenhum) ficaria ambígua entre dois tenants — gap achado e consertado nesta mesma rodada, antes de qualquer aplicação, via `portal_resolve_stripe_connect_tenant_service` (novo RPC, `service_role`-only) e um tratamento explícito de `unique_violation` em `portal_complete_stripe_connect_service` (`outcome: account_already_connected_elsewhere`, nunca uma exceção crua). `portal_apply_business_checkout_connect_event_service` cruza o `account` do evento assinado contra o `stripe_account_id` snapshotado na própria reserva (`outcome: ignored_account_mismatch` quando divergem): sem esse cruzamento, uma conta Standard conectada (com acesso ao próprio dashboard Stripe) poderia forjar uma Checkout Session com `metadata.tenant_id`/`reservation_id` de outro tenant e aplicar o desfecho a uma reserva alheia, já que a assinatura HMAC do endpoint é compartilhada entre todas as contas conectadas. `portal_schema_capabilities_service()` sobe para v69. Provado por `scripts/supabase-portal-integration.mjs` (`assertBusinessActionCheckoutStripeConnectPhase`) contra Postgres local: recusa declarada por conta Stripe não conectada, por `checkout_enabled=false` e por produto de catálogo inexistente; resolução de tenant por `stripe_account_id` e colisão entre dois tenants tentando conectar a mesma conta; ciclo completo reserve→approve→dispatch→commit→webhook até `payment_completed`; evento com `account` divergente rejeitado sem side effect, seguido do mesmo evento com o `account` certo aplicando normalmente; caminho de rejeição; `contact_email` exigido na aprovação quando nenhuma chamada anterior capturou um; varredura de expiração de 72h; idempotência por `grant_id` e por `event_id` do webhook. Nunca aplicada ao projeto hospedado; gates de pré-lançamento completos (habilitar Connect na conta Stripe da Axtro, revisão de segurança/compliance dedicada, percentual de taxa, tela de aprovação do `tenant_admin`) listados no próprio ADR-040. |
...
- O runtime recuperado reconhece v50, v56, v57 e v58 conforme as flags. Com o outbox desligado, v50/v56 exigem `meetingTerminalNotificationClaim=true`; v57/v58 exigem a claim antiga desabilitada e todas as capabilities da ADR-043. Com o outbox ligado, v57/v58 são aceitas. O preview textual continua fechado; somente v58 publica `portalTextPreviewAuthorityRepair=true`. A bridge de ações de negócio continua exigindo a união completa das capabilities de negócio na versão correspondente.
- Depois de aplicar v58, um artefato que não reconhece v58 não é rollback válido. O rollback suportado mantém build v58-aware, preview fechado, flag do outbox desligada e workflow suspenso, ou usa hotfix forward-only.
- O lote do worker M6-01 é limitado a 4 comandos sequenciais por lease de 60 segundos. O adapter usa uma mensagem batch separada por destinatário, e as capabilities caem para `false` se os triggers de payload imutável ou receipt append-only forem removidos.
- A versão hospedada atual não foi consultada nesta recuperação. Antes de promover, é obrigatório um preflight read-only do documento remoto e um plano de rollout e rollback compatível; não existe sobreposição automática entre o gate antigo de v48 e os gates v50/v56/v57/v58.
...
- A 0058 preserva a união de capabilities da v57 e substitui somente os fatos
  reparados do preview. A boundary recuperada deriva `auth.uid()`, mas permanece
  owner-only e sem `EXECUTE` para `PUBLIC`, `anon`, `authenticated` ou
  `service_role`; a primitiva histórica também perde esses grants. Nenhum caller
  externo envia identidade, tenant ou evidência jurídica em M6-02. Persistência
  `true` falha antes de qualquer escrita. Os wrappers rejeitam geração fora de 0 a 9 antes de
  claim, reserva, egress, transcript ou outbox. As FKs compostas impedem vínculo
  cross-tenant e usam `ON DELETE SET NULL (transcript_id)` para manter receipt
  sem conteúdo após exclusão. Isso não aplica a migration remotamente, não abre
  o preview, não opera provider e não completa retenção ou purge.
