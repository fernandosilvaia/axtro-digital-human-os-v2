...
- O runtime recuperado reconhece v50, v56 e v57 conforme as flags. Com o outbox desligado, v50/v56 exigem `meetingTerminalNotificationClaim=true`; v57 exige a claim antiga desabilitada e todas as capabilities da ADR-043. Com o outbox ligado, somente v57 é aceita. A bridge de ações de negócio continua exigindo a união completa das capabilities de negócio na versão correspondente.
- Depois de aplicar v57, um artefato anterior à v57 não é rollback válido: ele rejeita o schema enquanto os produtores PostgreSQL continuam enfileirando. O rollback suportado mantém build v57-aware, flag desligada e workflow suspenso, ou usa hotfix forward-only.
...
- A versão hospedada atual não foi consultada nesta recuperação. Antes de promover, é obrigatório um preflight read-only do documento remoto e um plano de rollout e rollback compatível; não existe sobreposição automática entre o gate antigo de v48 e o gate novo de v50/v56.
