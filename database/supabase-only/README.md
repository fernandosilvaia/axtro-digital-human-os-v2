...
| `0031_cost_alert_dispatches.sql` | **NÃO aplicado ainda** — tabela `tenant_cost_alerts` + RPC `portal_claim_cost_alert_service` (dedup de envio, D-V2-107) + `portal_billing_status` ganha `tenant_id` no retorno (campo aditivo). Aguardando autorização explícita do Fernando |
| `0032_search_knowledge_service.sql` | **NÃO aplicado ainda** — `portal_search_knowledge_service`, variante service-role de `portal_search_knowledge` (0010), fecha o gap de RAG no caminho de vídeo declarado desde D-V2-083 (D-V2-108). Aguardando autorização explícita do Fernando |
