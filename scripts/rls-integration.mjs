...
function assertCrossTenantRelationshipsAreRejected(runtimeUrl) {
  ...
  const crossTenantWorkflowSource = withTenant(runtimeUrl, fixture.tenantBeta, `INSERT INTO workflow_commands (tenant_id, id, session_id, workflow_type, workflow_version, aggregate_type, source_event_type, source_event_id, source_event_fingerprint, source_aggregate_version, source_state_hash, command_fingerprint, trace_id, correlation_id, causation_id, requested_by, idempotency_key, scheduled_at, created_at, data_classification) VALUES ('${fixture.tenantBeta}', '${fixture.crossWorkflowCommand}', '${fixture.sessionAlpha}', 'post_call_processing', '1.0.0', 'interaction_session', 'session.completed', '${fixture.timelineCompletionAlpha}', '${"a".repeat(64)}', 2, '${"a".repeat(64)}', '${"a".repeat(64)}', '0123456789abcdef0123456789abcdef', '${fixture.correlationCompletionAlpha}', '${fixture.timelineAlpha}', '${fixture.serviceIdentityAlpha}', 'post-call-processing/v1/${fixture.tenantBeta}/${fixture.timelineCompletionAlpha}', now(), now(), 'internal');`);
  assert.notEqual(crossTenantWorkflowSource.status, 0, "workflow command must not reference another tenant's session, completion event, or identity");
  const mismatchedWorkflowSessionSource = withTenant(runtimeUrl, fixture.tenantAlpha, `INSERT INTO workflow_commands (tenant_id, id, session_id, workflow_type, workflow_version, aggregate_type, source_event_type, source_event_id, source_event_fingerprint, source_aggregate_version, source_state_hash, command_fingerprint, trace_id, correlation_id, causation_id, requested_by, idempotency_key, scheduled_at, created_at, data_classification) VALUES ('${fixture.tenantAlpha}', '${fixture.mismatchedWorkflowCommand}', '${fixture.sessionAlpha}', 'post_call_processing', '1.0.0', 'interaction_session', 'session.completed', '${fixture.timelineCompletionOtherSession}', '${"a".repeat(64)}', 1, '${"a".repeat(64)}', '${"a".repeat(64)}', '0123456789abcdef0123456789abcdef', '${fixture.correlationCompletionOtherSession}', NULL, '${fixture.serviceIdentityAlpha}', 'post-call-processing/v1/${fixture.tenantAlpha}/${fixture.timelineCompletionOtherSession}', now(), now(), 'internal');`);
  assert.notEqual(mismatchedWorkflowSessionSource.status, 0, "workflow command source event must belong to the same session");
  const mismatchedWorkflowReceipt = withTenant(runtimeUrl, fixture.tenantAlpha, `INSERT INTO workflow_step_receipts (tenant_id, id, session_id, workflow_run_id, command_id, source_event_id, source_aggregate_version, source_event_type, step, attempt, outcome, artifact_hash, failure_code, fencing_token, state_version_before, state_version_after, trace_id, correlation_id, started_at, completed_at, data_classification) VALUES ('${fixture.tenantAlpha}', '${fixture.mismatchedWorkflowReceipt}', '${fixture.sessionAlphaOther}', '${fixture.postCallRunAlpha}', '${fixture.postCallCommandOtherSession}', '${fixture.timelineCompletionOtherSession}', 1, 'session.completed', 'generate_summary', 1, 'checkpointed', '${"d".repeat(64)}', NULL, '${fixture.mismatchedWorkflowFence}', 1, 2, 'fedcba9876543210fedcba9876543210', '${fixture.correlationCompletionOtherSession}', '2026-07-14T00:00:01.000Z', '2026-07-14T00:00:02.000Z', 'internal');`);
  assert.notEqual(mismatchedWorkflowReceipt.status, 0, "workflow receipt must bind one matching run, command, session, and source");
  const mismatchedWorkflowResult = withTenant(runtimeUrl, fixture.tenantAlpha, `INSERT INTO post_call_workflow_results (tenant_id, workflow_run_id, session_id, command_id, source_event_id, source_event_fingerprint, source_aggregate_version, source_event_type, source_state_hash, trace_id, correlation_id, causation_id, summary_template_code, summary_text, canonical_event_count, final_state_version, evaluator_version, evaluation_outcome, score_basis_points, follow_up_command_id, follow_up_mode, follow_up_status, follow_up_external_effect, follow_up_effect_hash, result_hash, completed_at, data_classification) VALUES ('${fixture.tenantAlpha}', '${fixture.postCallRunOtherSession}', '${fixture.sessionForDeletion}', '${fixture.postCallCommandOtherSession}', '${fixture.timelineCompletionDeletionSession}', '${"a".repeat(64)}', 1, 'session.completed', '${"a".repeat(64)}', 'abcdef0123456789abcdef0123456789', '${fixture.correlationCompletionDeletionSession}', NULL, 'deterministic_session_summary_v1', 'Mismatched same-tenant workflow result fixture.', 1, 1, 'fake-structural-v1', 'passed', 10000, '${fixture.mismatchedFollowUpCommand}', 'deterministic_noop', 'not_sent', false, '${"c".repeat(64)}', '${"b".repeat(64)}', '2026-07-14T00:00:02.000Z', 'restricted');`);
  assert.notEqual(mismatchedWorkflowResult.status, 0, "workflow result must bind one matching run, command, session, and source");
  const mismatchedWorkflowEvidence = withTenant(runtimeUrl, fixture.tenantAlpha, `INSERT INTO post_call_workflow_result_evidence (tenant_id, workflow_run_id, session_id, ordinal, evidence_event_id, evidence_aggregate_version, evidence_event_type) VALUES ('${fixture.tenantAlpha}', '${fixture.postCallRunAlpha}', '${fixture.sessionAlphaOther}', 1, '${fixture.timelineCompletionOtherSession}', 1, 'session.completed');`);
  assert.notEqual(mismatchedWorkflowEvidence.status, 0, "workflow result evidence must belong to the result session");
  for (const [mutation, message] of [
    ["attempts = 0", "completed workflow must retain a positive attempt"],
    ["started_at = NULL", "completed workflow must retain its start timestamp"],
    ["current_step = 'evaluate'", "completed workflow must remain on finalize"],
    ["cancelled_at = completed_at", "completed workflow cannot also be cancelled"],
  ]) {
    const invalidLifecycle = withTenant(
      runtimeUrl,
      fixture.tenantAlpha,
      `UPDATE workflow_runs SET ${mutation} WHERE tenant_id = '${fixture.tenantAlpha}' AND id = '${fixture.postCallRunAlpha}';`,
    );
    assert.notEqual(invalidLifecycle.status, 0, message);
  }
}
...
function assertAppendOnlyTablesRejectMutation(runtimeUrl) {
  const mutations = [
    ...
    ["workflow_commands", fixture.postCallCommandAlpha, "UPDATE workflow_commands SET source_event_type = 'tampered'"],
    ["workflow_step_receipts", fixture.workflowStepReceiptAlpha, "UPDATE workflow_step_receipts SET outcome = 'failed'"],
  ];
  ...
  const resultMutation = withTenant(runtimeUrl, fixture.tenantAlpha, `UPDATE post_call_workflow_results SET summary_text = 'tampered' WHERE tenant_id = '${fixture.tenantAlpha}' AND workflow_run_id = '${fixture.postCallRunAlpha}';`);
  assert.notEqual(resultMutation.status, 0, "post_call_workflow_results must reject updates");
  const evidenceDeletion = withTenant(runtimeUrl, fixture.tenantAlpha, `DELETE FROM post_call_workflow_result_evidence WHERE tenant_id = '${fixture.tenantAlpha}' AND workflow_run_id = '${fixture.postCallRunAlpha}' AND evidence_event_id = '${fixture.timelineCompletionAlpha}';`);
  assert.notEqual(evidenceDeletion.status, 0, "post_call_workflow_result_evidence must reject deletes");
}
...
function seedSql(f) {
  ...
  return `
    ...
    INSERT INTO service_identities (tenant_id, id, name, identity_type, status) VALUES ('${f.tenantAlpha}', '${f.serviceIdentityAlpha}', 'workflow-runtime', 'workflow', 'active');
    ...
    INSERT INTO session_timeline (tenant_id, id, event_id, session_id, aggregate_version, event_type, event_version, event_document, trace_id, correlation_id, causation_id, occurred_at) VALUES ('${f.tenantAlpha}', '${f.timelineCompletionAlpha}', '${f.timelineCompletionAlpha}', '${f.sessionAlpha}', 2, 'session.completed', 1, jsonb_build_object('schema_version', '2.0.0', 'event_id', '${f.timelineCompletionAlpha}', 'event_type', 'session.completed', 'event_version', 1, 'aggregate_type', 'interaction_session', 'aggregate_id', '${f.sessionAlpha}', 'aggregate_version', 2, 'tenant_id', '${f.tenantAlpha}', 'session_id', '${f.sessionAlpha}', 'producer', 'rls-fixture', 'trace_id', '0123456789abcdef0123456789abcdef', 'correlation_id', '${f.correlationCompletionAlpha}', 'causation_id', '${f.timelineAlpha}', 'data_classification', 'internal', 'payload_json', '{}', 'occurred_at', '2026-07-14T00:00:01.000Z'), '0123456789abcdef0123456789abcdef', '${f.correlationCompletionAlpha}', '${f.timelineAlpha}', '2026-07-14T00:00:01.000Z');
    INSERT INTO session_timeline (tenant_id, id, event_id, session_id, aggregate_version, event_type, event_version, event_document, trace_id, correlation_id, causation_id, occurred_at) VALUES ('${f.tenantAlpha}', '${f.timelineCompletionOtherSession}', '${f.timelineCompletionOtherSession}', '${f.sessionAlphaOther}', 1, 'session.completed', 1, jsonb_build_object('schema_version', '2.0.0', 'event_id', '${f.timelineCompletionOtherSession}', 'event_type', 'session.completed', 'event_version', 1, 'aggregate_type', 'interaction_session', 'aggregate_id', '${f.sessionAlphaOther}', 'aggregate_version', 1, 'tenant_id', '${f.tenantAlpha}', 'session_id', '${f.sessionAlphaOther}', 'producer', 'rls-fixture', 'trace_id', 'fedcba9876543210fedcba9876543210', 'correlation_id', '${f.correlationCompletionOtherSession}', 'causation_id', NULL, 'data_classification', 'internal', 'payload_json', '{}', 'occurred_at', '2026-07-14T00:00:01.000Z'), 'fedcba9876543210fedcba9876543210', '${f.correlationCompletionOtherSession}', NULL, '2026-07-14T00:00:01.000Z');
    INSERT INTO session_timeline (tenant_id, id, event_id, session_id, aggregate_version, event_type, event_version, event_document, trace_id, correlation_id, causation_id, occurred_at) VALUES ('${f.tenantAlpha}', '${f.timelineCompletionDeletionSession}', '${f.timelineCompletionDeletionSession}', '${f.sessionForDeletion}', 1, 'session.completed', 1, jsonb_build_object('schema_version', '2.0.0', 'event_id', '${f.timelineCompletionDeletionSession}', 'event_type', 'session.completed', 'event_version', 1, 'aggregate_type', 'interaction_session', 'aggregate_id', '${f.sessionForDeletion}', 'aggregate_version', 1, 'tenant_id', '${f.tenantAlpha}', 'session_id', '${f.sessionForDeletion}', 'producer', 'rls-fixture', 'trace_id', 'abcdef0123456789abcdef0123456789', 'correlation_id', '${f.correlationCompletionDeletionSession}', 'causation_id', NULL, 'data_classification', 'internal', 'payload_json', '{}', 'occurred_at', '2026-07-14T00:00:01.000Z'), 'abcdef0123456789abcdef0123456789', '${f.correlationCompletionDeletionSession}', NULL, '2026-07-14T00:00:01.000Z');
    ...
    INSERT INTO workflow_commands (tenant_id, id, session_id, workflow_type, workflow_version, aggregate_type, source_event_type, source_event_id, source_event_fingerprint, source_aggregate_version, source_state_hash, command_fingerprint, trace_id, correlation_id, causation_id, requested_by, idempotency_key, scheduled_at, created_at, data_classification) VALUES ('${f.tenantAlpha}', '${f.postCallCommandAlpha}', '${f.sessionAlpha}', 'post_call_processing', '1.0.0', 'interaction_session', 'session.completed', '${f.timelineCompletionAlpha}', '${hash}', 2, '${hash}', '${hash}', '0123456789abcdef0123456789abcdef', '${f.correlationCompletionAlpha}', '${f.timelineAlpha}', '${f.serviceIdentityAlpha}', 'post-call-processing/v1/${f.tenantAlpha}/${f.timelineCompletionAlpha}', '2026-07-14T00:00:01.000Z', '2026-07-14T00:00:01.000Z', 'internal');
    INSERT INTO workflow_commands (tenant_id, id, session_id, workflow_type, workflow_version, aggregate_type, source_event_type, source_event_id, source_event_fingerprint, source_aggregate_version, source_state_hash, command_fingerprint, trace_id, correlation_id, causation_id, requested_by, idempotency_key, scheduled_at, created_at, data_classification) VALUES ('${f.tenantAlpha}', '${f.postCallCommandOtherSession}', '${f.sessionAlphaOther}', 'post_call_processing', '1.0.0', 'interaction_session', 'session.completed', '${f.timelineCompletionOtherSession}', '${hash}', 1, '${hash}', '${hash}', 'fedcba9876543210fedcba9876543210', '${f.correlationCompletionOtherSession}', NULL, '${f.serviceIdentityAlpha}', 'post-call-processing/v1/${f.tenantAlpha}/${f.timelineCompletionOtherSession}', '2026-07-14T00:00:01.000Z', '2026-07-14T00:00:01.000Z', 'internal');
    INSERT INTO workflow_runs (tenant_id, id, command_id, post_call_command_id, workflow_type, workflow_version, aggregate_type, aggregate_id, idempotency_key, status, current_step, attempts, input_document, started_at, updated_at, completed_at, state_version, max_attempts, result_hash, data_classification) VALUES ('${f.tenantAlpha}', '${f.postCallRunAlpha}', '${f.postCallCommandAlpha}', '${f.postCallCommandAlpha}', 'post_call_processing', '1.0.0', 'interaction_session', '${f.sessionAlpha}', 'post-call-processing/v1/${f.tenantAlpha}/${f.timelineCompletionAlpha}', 'completed', 'finalize', 1, '{}'::jsonb, '2026-07-14T00:00:01.000Z', '2026-07-14T00:00:02.000Z', '2026-07-14T00:00:02.000Z', 8, 4, '${"b".repeat(64)}', 'internal');
    INSERT INTO workflow_runs (tenant_id, id, command_id, post_call_command_id, workflow_type, workflow_version, aggregate_type, aggregate_id, idempotency_key, status, current_step, attempts, input_document, updated_at, state_version, max_attempts, data_classification) VALUES ('${f.tenantAlpha}', '${f.postCallRunOtherSession}', '${f.postCallCommandOtherSession}', '${f.postCallCommandOtherSession}', 'post_call_processing', '1.0.0', 'interaction_session', '${f.sessionAlphaOther}', 'post-call-processing/v1/${f.tenantAlpha}/${f.timelineCompletionOtherSession}', 'queued', 'generate_summary', 0, '{}'::jsonb, '2026-07-14T00:00:01.000Z', 1, 4, 'internal');
    INSERT INTO workflow_step_receipts (tenant_id, id, session_id, workflow_run_id, command_id, source_event_id, source_aggregate_version, source_event_type, step, attempt, outcome, artifact_hash, failure_code, fencing_token, state_version_before, state_version_after, trace_id, correlation_id, started_at, completed_at, data_classification) VALUES ('${f.tenantAlpha}', '${f.workflowStepReceiptAlpha}', '${f.sessionAlpha}', '${f.postCallRunAlpha}', '${f.postCallCommandAlpha}', '${f.timelineCompletionAlpha}', 2, 'session.completed', 'finalize', 1, 'completed', NULL, NULL, '${f.workflowFencingTokenAlpha}', 7, 8, '0123456789abcdef0123456789abcdef', '${f.correlationCompletionAlpha}', '2026-07-14T00:00:01.500Z', '2026-07-14T00:00:02.000Z', 'internal');
    INSERT INTO post_call_workflow_results (tenant_id, workflow_run_id, session_id, command_id, source_event_id, source_event_fingerprint, source_aggregate_version, source_event_type, source_state_hash, trace_id, correlation_id, causation_id, summary_template_code, summary_text, canonical_event_count, final_state_version, evaluator_version, evaluation_outcome, score_basis_points, follow_up_command_id, follow_up_mode, follow_up_status, follow_up_external_effect, follow_up_effect_hash, result_hash, completed_at, data_classification) VALUES ('${f.tenantAlpha}', '${f.postCallRunAlpha}', '${f.sessionAlpha}', '${f.postCallCommandAlpha}', '${f.timelineCompletionAlpha}', '${hash}', 2, 'session.completed', '${hash}', '0123456789abcdef0123456789abcdef', '${f.correlationCompletionAlpha}', '${f.timelineAlpha}', 'deterministic_session_summary_v1', 'Deterministic summary from two canonical session events.', 2, 2, 'fake-structural-v1', 'passed', 10000, '${f.followUpCommandAlpha}', 'deterministic_noop', 'not_sent', false, '${"c".repeat(64)}', '${"b".repeat(64)}', '2026-07-14T00:00:02.000Z', 'restricted');
    INSERT INTO post_call_workflow_result_evidence (tenant_id, workflow_run_id, session_id, ordinal, evidence_event_id, evidence_aggregate_version, evidence_event_type) VALUES ('${f.tenantAlpha}', '${f.postCallRunAlpha}', '${f.sessionAlpha}', 0, '${f.timelineCompletionAlpha}', 2, 'session.completed');
...
function createFixture() {
  ...
  return Object.freeze({
    ...
    timelineCompletionAlpha: nextId(), correlationCompletionAlpha: nextId(), postCallCommandAlpha: nextId(), postCallRunAlpha: nextId(),
    workflowStepReceiptAlpha: nextId(), workflowFencingTokenAlpha: nextId(), followUpCommandAlpha: nextId(), crossWorkflowCommand: nextId(),
    timelineCompletionOtherSession: nextId(), correlationCompletionOtherSession: nextId(), mismatchedWorkflowCommand: nextId(),
    postCallCommandOtherSession: nextId(), postCallRunOtherSession: nextId(), mismatchedWorkflowReceipt: nextId(),
    mismatchedWorkflowFence: nextId(), timelineCompletionDeletionSession: nextId(), correlationCompletionDeletionSession: nextId(),
    mismatchedFollowUpCommand: nextId(),
  });
}
