...
**Estado atual:** M0, M1, M2 e M3 concluídos (M3-01 a M3-09 fake-first/dry-run completos; M3-10 com ferramenta pronta, piloto real e bake-off de provider pendentes de gate humano); VISUAL-01 concluído; Cérebro Método Silva no ar (D-V2-073/074); SEO-AEO-01 concluído; rate card de custos no ar (D-V2-078); M4 (cérebro customizado próprio na persona de vídeo) com CÓDIGO completo (M4-01 a M4-04) e testado, mas nada ligado em produção — três dependências externas pendentes (ver Pendências externas)

**Marco atual:** M4 (código completo, gate humano pendente antes de qualquer rewiring de persona real)
**Tarefa atual:** nenhuma — aguardando o Fernando decidir sobre as 3 pendências externas antes de ligar o cérebro numa persona Tavus real
**Última evidência verde:** M4-04 em 2026-07-27 — `/api/brain/[agentId]/chat/completions` (SSE OpenAI-compatible) sobre `handle-chat-request.ts` (auth por bearer, degradação Art. 14 nunca derruba a call), 10 testes novos (467 Node total) + 26 Python, `next build` real gera a rota, typecheck, lint e 9 validadores verdes. RAG real é gap declarado (retorna vazio); nada aplicado em produção nem ligado a persona real
...
| `M4-04` | M4 | done (código, não ligado em produção) | Add Tavus-facing brain chat-completions HTTP route | `M4-01`, `M4-02`, `M4-03` | `/api/brain/[agentId]/chat/completions` (SSE), `handle-chat-request.ts` com 10 testes novos (467 Node total), `next build` real gera a rota; `0019_agent_brain_service_role_rpcs.sql` (mesmo bloqueio de aplicação do M4-03) + `SUPABASE_SERVICE_ROLE_KEY` (nunca configurada, dependência externa nova) faltam para funcionar de verdade; RAG real (`portal_search_knowledge` exige `auth.uid()`) é gap declarado, retorna vazio por enquanto; nenhuma persona Tavus real aponta pra cá |
