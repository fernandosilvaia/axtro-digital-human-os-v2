...
| 4 | Confirmação de e-mail em signup real depende de SMTP do Supabase (padrão: e-mail builtin com rate limit baixo). Para beta com clientes, configurar SMTP próprio | — | Fernando |
...
| 3 | `user_tenant_memberships` permite um único tenant por usuário (PK em `user_id`) | Suficiente para self-serve atual | Multi-tenant por usuário/convites exigem nova modelagem (revisit trigger no ADR-032) |
